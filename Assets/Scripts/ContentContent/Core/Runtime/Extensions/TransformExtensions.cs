...
namespace ContentContent
{
    ...
    public static class TransformExtensions
    {
        ...
        private static void GetFullPath(StringBuilder stringBuilder, Transform transform, string delimiter,
            string prefix)
        {
        ...
        /// </summary>
        /// <param name="root">Start point of the traverse set</param>
        ...
        {
            Queue<Transform> transformQueue = new();
        ...
        /// </returns>
        public static Bounds GetColliderBounds(this Transform transform, ref Collider[] colliders,
        ...
        /// </returns>
        public static Bounds GetRenderBounds(this Transform transform, ref Renderer[] renderers,
            bool syncTransform = true)
        {
            // Store current rotation then zero out the rotation so that the bounds
            // are computed when the object is in its 'axis aligned orientation'.
        ...
        {
            if (!includeSelf) startTransform = startTransform.parent;
        ...
        {
            Vector3 transformedSize = localSize;
            Transform transformedCopy = transform;

            do
            {
                Vector3 localScale = transformedCopy.localScale;
        ...
        {
            Vector3 transformedSize = worldSize;
            Transform transformedCopy = transform;
        ...
        public static bool TryGetDepth(Transform target, Transform parent, ref int depth)
        {
            for (var i = 0; i < parent.childCount; i++)
            {
                ...
                if (child == target.transform || TryGetDepth(target, child, ref depth)) return true;
            }
        ...
        public static Vector3 GetPointOnBoundsEdge(this Transform transform, Vector3 direction, Bounds bounds = default,
            Collider[] cachedColliders = null)
        {
            if (direction != Vector3.zero)
                direction /=
                    Mathf.Max(Mathf.Max(Mathf.Abs(direction.x), Mathf.Abs(direction.y), Mathf.Abs(direction.y)));

            if (bounds == default) bounds = transform.GetColliderBounds(ref cachedColliders);
        ...
        /// <returns>
        ///     The collection of colliders that were acted on. This collection can be used later when calling this method
        ///     again.
        /// </returns>
        public static Collider[] SetCollidersActive(this Transform transform, bool isActive,
            Collider[] colliders = null)
        {
            colliders ??= transform.GetComponentsInChildren<Collider>();

            foreach (Collider collider in colliders) collider.enabled = isActive;
        ...
        {
            Vector3 point = target.localPosition;
            Vector3 distance = point - pivot; // diff from object pivot to desired pivot/origin
            float relativeScale = newScale.x / target.localScale.x; // relative scale factor
            Vector3 finalPosition = pivot + distance * relativeScale; // calc final position post-scale
